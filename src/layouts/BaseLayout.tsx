...
const BaseLayout: React.FC = observer(() => {
  ...
  const asideWidth = appStore.collapse ? (pathname && pathname.indexOf('/design') !== -1 ? 'sider-zero-width' : 'sider-collapse-width') : 'sider-width';
  const asideBoardVisible = !(pathname === '/design' && appStore.collapse);
  return (
    <div className={classNames('size-full flex transition-all-300')}>
      <aside className={classNames('h-full overflow-hidden transition-all-300', asideWidth, { 'sider-border-r': asideBoardVisible })}>
